...
@st.cache_data(show_spinner=True)
def get_player_season_averages(player_id: int, season: str):
    """
...
@st.cache_data(show_spinner=True)
def get_team_players_with_stats(team_id: int, season: str, min_avg_minutes: float = 25.0):
    """
    ...
    for _, row in roster_df.iterrows():
        ...
        stats = get_player_season_averages(player_id, season)
        games = stats["games"]
        if games == 0:
            continue
...
def main():
    ...
    with st.spinner("Fetching roster and season stats from NBA API..."):
        try:
